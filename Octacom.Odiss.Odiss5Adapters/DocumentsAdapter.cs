...
{
    public class DocumentsAdapter : IDocumentsAdapter
    {
        private readonly IServiceProvider serviceProvider;

        private IDictionary<Guid, Type> ApplicationDocumentTypeMappings { get; set; }

        public DocumentsAdapter(IServiceProvider serviceProvider)
        {
        ...
        public dynamic SearchAutoComplete(string query, App app, string mapTo)
        {
            var adapter = GetAdapterForApplication(app.ID);
            var adapterType = adapter.GetType();
            var getResultsMethod = adapterType.GetMethod("SearchAutoComplete");
            return getResultsMethod.Invoke(adapter, new object[] { query, app, mapTo });
        }

        public void SetMappings(IDictionary<Guid, Type> mappings)
        {
            ApplicationDocumentTypeMappings = mappings;
        }

        private object GetAdapterForApplication(Guid appId)
        {
            if (!ApplicationDocumentTypeMappings.ContainsKey(appId))
            {
                throw new Exception($"No IDocumentsAdapter found for application ID ${appId}. Entity Type is not mapped.");
            }

            var type = ApplicationDocumentTypeMappings[appId];
            var adapterType = typeof(IDocumentsAdapter<>).MakeGenericType(type);

            return serviceProvider.GetService(adapterType);
        }
    }

    public class DocumentsAdapter<TDocument> : IDocumentsAdapter<TDocument>
    {
        ...
        public DocumentsAdapter(ISearchEngine<TDocument> documentSearchEngine, GlobalSearchEngine searchEngine, SearchEngineRegistry searchEngineRegistry)
        {
        ...
        public dynamic SearchAutoComplete(string query, App app, string mapTo)
        {
            ...
            var searchOptions = CreateEntitySearchOptions(searchObject, query);
        ...
        private static GlobalSearchOptions CreateEntitySearchOptions(JObject searchObject, string query)
        {
